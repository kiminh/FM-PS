...
class Network(object):
  ...
  def get_current_gradient(self, gradient):
    for l in range(-1,-len(self.layers),-1):
      #对于每一个可能存在的参数提取
      layer = self.layer[l]
      if hasattr(layer, "w_name"):
        gradient[layer.w_name] = layer.one_batch_dw
      if hasattr(layer, "b_name"):
        gradient[layer.b_name] = layer.one_batch_db
      #TODO扩展其他参数
